...
*/
	...
	ofstream file;
	file.open("ispis.txt");
	file << "#iter.\t ime \t mass \t bozon \t px0 \t py0 \t pz0 \t px1 \t py1 \t pz1 \t px2 \t py2 \t pz2" << endl;
	for(int i=0; i<N;i++){
		...
		c1->Ispis();
		c2->Ispis();
		file << i << ". \t"<< c1->_ime << "\t" << c1->_mass << "\t" << c1->_bozon << "\t" << c0->p1 << "\t" << c0->p2 << "\t" << c0->p3 << "\t" << c1->p1 << "\t" << c1->p2 << "\t" << c1->p3 << "\t" << c2->p1 << "\t" << c2->p2 << "\t" << c2->p3 << endl;
 		}
	file.close();
	return 0;
